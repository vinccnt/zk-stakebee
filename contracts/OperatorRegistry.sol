...
contract OperatorRegistry is Owned {
    ...
    address[] validators; // Array of unused / undeposited validators that can be used at a future time
    ...
    function addValidator(address validator) public onlyByOwnGov {
    ...
    /** @dev You should verify offchain that the validators are indeed valid before adding them
        Reason we don't do that here is for gas */
    function addValidators(address[] calldata validatorArray) external onlyByOwnGov {
    ...
    function swapValidator(uint256 from_idx, uint256 to_idx) public onlyByOwnGov {
        ...
        address fromVal = validators[from_idx];
        address toVal = validators[to_idx];

        // Set the swapped values
        validators[to_idx] = fromVal;
        validators[from_idx] = toVal;

        emit ValidatorsSwapped(fromVal, toVal, from_idx, to_idx);
    }
    ...
    function removeValidator(uint256 remove_idx, bool dont_care_about_ordering) public onlyByOwnGov {
        ...
        address removed_address = validators[remove_idx];
        ...
        // More gassy, loop
            ...
            address[] memory original_validators = validators;
        ...
        emit ValidatorRemoved(removed_address, remove_idx, dont_care_about_ordering);
    }
        ...
        )
    {
        ...
        address popped = validators[numVals - 1];
        validators.pop();
    ...
    event ValidatorAdded(address addr);
    event ValidatorArrayCleared();
    event ValidatorRemoved(address addr, uint256 remove_idx, bool dont_care_about_ordering);
    event ValidatorsPopped(uint256 times);
    event ValidatorsSwapped(address from_addr, address to_addr, uint256 from_idx, uint256 to_idx);
    event KeysCleared();
}
